...
Fait mesuré sur le run `20260814-tests-sfr` (FastAPI + Jinja, ledger seq 6-7) : le produit
déclare `FORGE_TESTS_PRODUIT=Ventilation de facture SFR` dans `<projet>/.env.forge-tests` —
...
_ENV_DOCUMENTE = """\
FORGE_TESTS_PRODUIT=Ventilation de facture SFR
FORGE_TESTS_BASE_URL=https://compta-qualif.exemple.test
FORGE_TESTS_SOURCES=C:\\dev\\compta-sfr
...
def test_le_nom_du_produit_declare_n_entre_plus_au_corpus(tmp_path: Path, env_propre) -> None:
    ...
    assert "Ventilation de facture SFR" not in interdites
...
def test_le_dashboard_masque_toujours_le_mot_de_passe_et_jamais_le_nom(
    ...
    assert "Ventilation de facture SFR" not in secrets
...
def test_le_jeu_de_donnees_du_produit_declare_passe_le_garde_fou(
    ...
    jeu = jeux.construire([], "Ventilation de facture SFR")

    jeux.verifier(jeu, projet)  # AVANT TF-0215 : DonneeNonSynthetique, exit 2

    assert jeu["produit"] == "Ventilation de facture SFR"
...
def test_les_quatre_livrables_sont_produits_avec_la_configuration_documentee(
    ...
    assert all(
        chemin.name.startswith("Ventilation de facture SFR - ") for chemin in chemins.values()
    )
