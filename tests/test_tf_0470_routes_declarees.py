...
def test_sans_declaration_le_pan_DIT_ce_qu_il_ne_juge_pas(tmp_path, monkeypatch) -> None:
    ...
    assert any("AUCUNE route declaree" in l for l in sortie.non_juge)
    assert any("FORGE_TESTS_I18N_ROUTES" in l for l in sortie.non_juge)
    assert any("standalone" in l for l in sortie.non_juge)
